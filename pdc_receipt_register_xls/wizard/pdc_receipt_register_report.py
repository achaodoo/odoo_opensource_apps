...
class PDCReceiptRegisterReport(models.TransientModel):
    _name = "pdc.rec.reg.report"
    _description = "PDC Receipt Register Report"
    ...
    def action_generate_pdc_receipt_register_report(self):
        ...
        sheet.merge_range(row_number, column_number, row_number, ending_range - 1,
                          "PDC Receipt Register", title_format)
        row_number += 1
        sheet.merge_range(row_number, column_number, row_number, ending_range - 1,
        ...
        for pay in bank_payments:
            ...
            sheet.write(
                row, col + 5,
                pay.journal_id.bank_account_id.acc_number
                if pay.journal_id.bank_account_id else '',
                line_format_left
            )

            sheet.write(
                row, col + 6,
                pay.cheque_date.strftime("%d-%m-%Y")
                if pay.cheque_date else '',
                line_format_left
            )
            ...
            row += 1
        sheet.write(row, col + 6, 'Total ' + str(self.env.company.currency_id.name), total_line_format_right)
        sheet.write(row, col + 7, "{:0.2f}".format(total_amount), total_line_format_right)
        ...
        return {'type': 'ir.actions.act_url',
                'url': '/web/binary/export_pdc_receipt_register_report?id=%s&model=pdc.rec.reg.report' % (
                    self.id),
                'target': 'new'}
